  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-white">Vendors Management</h2>
        <button onClick={fetchVendors} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg">Refresh</button>
      </div>
      {loading ? (
          ...
          <table className="min-w-full bg-gray-900 rounded-lg">
            <thead>
              <tr className="border-b border-gray-800">
                ...
                <th className="px-4 py-3 text-left text-white">Hourly Rate</th>
            ...
            <tbody>
              {vendors.map(vendor => (
                <tr key={vendor._id} className="border-b border-gray-800">
                  <td className="px-4 py-3 text-gray-200">{vendor.name}</td>
                  <td className="px-4 py-3 text-gray-200">{vendor.email}</td>
                  <td className="px-4 py-3">
                    <span className="px-2 py-1 rounded-full text-xs font-medium border bg-indigo-900/30 text-indigo-300 border-indigo-700">{vendor.category}</span>
                  </td>
                  <td className="px-4 py-3 text-gray-200">${vendor.hourlyRate}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium border ${vendor.isAccountVerified ? 'bg-emerald-900/30 text-emerald-300 border-emerald-700' : 'bg-yellow-900/30 text-yellow-300 border-yellow-700'}`}>
                      {vendor.isAccountVerified ? 'Verified' : 'Pending'}
                    </span>
                  </td>
                  <td className="px-4 py-3 flex gap-2">
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        toast.info(
                          <div>
                            <div className="mb-2">Are you sure you want to ban this vendor? They will be blocked from login and registration.</div>
                            <div className="flex gap-2 justify-end">
                              <button
                                className="px-3 py-1 bg-yellow-600 text-white rounded hover:bg-yellow-700 text-xs"
                                onClick={async () => {
                                  toast.dismiss();
                                  try {
                                    await axios.post('/api/vendor/admin/ban', { email: vendor.email, reason });
                    ...
                    <button
                      ...
                      onClick={() => {
                        toast.info(
                          <div>
                            <div className="mb-2">Unban this vendor?</div>
                            <div className="flex gap-2 justify-end">
                              <button
                                className="px-3 py-1 bg-amber-700 text-white rounded hover:bg-amber-800 text-xs"
                                onClick={async () => {
                                  toast.dismiss();
                                  try {
                                    await axios.post('/api/vendor/admin/unban', { email: vendor.email });
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const openModal = () => {
                          toast.info(
                            <div>
                              <div className="mb-2 font-semibold text-white">Send Note to {vendor.name}</div>
                              ...
                              <div className="flex gap-2 justify-end mt-2">
                                ...
                                <button className="px-3 py-1 bg-indigo-600 text-white rounded hover:bg-indigo-700 text-xs" onClick={async ()=>{
                                  try {
                                    ...
                                    await axios.post('/api/vendor/admin/notice', { vendorId: vendor._id, message });
                                    toast.dismiss();
